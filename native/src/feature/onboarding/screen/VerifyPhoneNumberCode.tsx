...
import { FooterText, PrimaryText, SecondaryText } from '../../../common/component/Text';
...
type Props = {
	navigation: StackNavigationProp<OnboardingStackParamList, OnboardingStackScreenName.VerifyPhoneNumberCode>;
	route: RouteProp<OnboardingStackParamList, OnboardingStackScreenName.VerifyPhoneNumberCode>;
};

export const VerifyPhoneNumberCode: React.FC<Props> = ({ route, navigation }) => {
	...
	const { executeVerifyPhoneNumberCode, result, error } = onboarding.useVerifyPhoneNumberCode();
	...
	const handleError = () => {
		Alert.alert('Oops!', error?.message, [{ text: 'Entendido', onPress: () => null }]);
	};
