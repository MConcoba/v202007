...
export const sendPhoneNumberVerificationCode: QueryResolvers<IContext>['sendPhoneNumberVerificationCode'] = async (
	parent,
	{ input },
	{ dependencies }
) => {
	try {
		console.log(input);
		const phoneNumberVerificationMiddleware = dependencies.provide(PhoneNumberVerificationMiddlewareInjectionKey);
		...
		const phoneNumberExistsMiddleware = dependencies.provide(PhoneNumberExistsMiddlewareInjectionKey);

		if (!(await phoneNumberExistsMiddleware.isValid(input))) {
			throw phoneNumberExistsMiddlewareError.phoneNumberExistsError;
		}

		const onboardingController = dependencies.provide(OnboardingControllerInjectionKey);
